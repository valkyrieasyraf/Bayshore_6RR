		...
		app.post('/method/load_user', async (req, res) => {
			...
			if (!user) {
				...
				if (newCardsBanned === 0) {
					let user = await prisma.user.create({
						data: {
							...
							tutorials: [
								...
								false, // TUTORIAL_ID_GHOST_UNUSED_53 = 53
								false, // TUTORIAL_ID_GHOST_GHOST_BINGO_LINK = 54
								false, // TUTORIAL_ID_GHOST_GHOST_KOSHIEN_NEW = 55
								false, // TUTORIAL_ID_GHOST_GHOST_KOSHIEN_NOTICE = 56
								false, // TUTORIAL_ID_GHOST_GHOST_KOSHIEN_MVP_UPDATE = 57
								false, // TUTORIAL_ID_GHOST_BINGO_LUCKY_CAHNCE = 58
								false, // TUTORIAL_ID_GHOST_GHOST_KOSHIEN_FINISHED = 59
								false, // TUTORIAL_ID_GHOST_GHOST_KOSHIEN_AFTEREVENT = 60
								false, // TUTORIAL_ID_GHOST_GHOST_BINGO_AREA = 61
							],
						}
					});
					...
					//Gift Special Cars Ticket
					let spTicketCars = Config.getConfig().gameOptions.giftCarsTicketToNewUsers;

					if (spTicketCars > 0) {
						console.log(`Gift Special Cars Ticket to new user...`);
						await prisma.userItem.create({
							data: {
								userId: user.id,
								category: wm.wm.protobuf.ItemCategory.CAT_CAR_TICKET,
								itemId: 1, // HIACE VAN
								type: 1 // Special Cars Ticket
							}
						});
						...
						await prisma.userItem.create({
							data: {
								userId: user.id,
								category: wm.wm.protobuf.ItemCategory.CAT_CAR_TICKET,
								itemId: 6, // FAIRLADY Z nismo Black
								type: 1 // Special Cars Ticket
							}
						});
						...
						await prisma.userItem.create({
							data: {
								userId: user.id,
								category: wm.wm.protobuf.ItemCategory.CAT_CAR_TICKET,
								itemId: 42, // CAMARO SS RS Black Edition
								type: 1 // Special Cars Ticket
							}
						});
						...
						await prisma.userItem.create({
							data: {
								userId: user.id,
								category: wm.wm.protobuf.ItemCategory.CAT_CAR_TICKET,
								itemId: 44, // ROADSTER S Special Package
								type: 1 // Special Cars Ticket
							}
						});
