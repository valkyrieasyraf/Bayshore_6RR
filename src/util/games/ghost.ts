...
export async function saveGhostBattleResult(body: wm.protobuf.SaveGameResultRequest, car: any)
{
    ...
    if (!(body.retired || body.timeup)) 
    {
        ...
        if (body.earnedUserItems.length !== 0) { 
            ...
            console.log('No bingo reward available.');
        }        
        ...
        if (ghostResult)
        {
            ...
            // bingo save (find in db first and then from the game)
            let getBingoNumbersDB = await prisma.car.findFirst({
                where: {
                    carId: body.carId
                },
                select: {
        ...
        switch (body.rgResult!.selectionMethod) 
        {
            ...
            // Ghost Battle Bingo (26)
            case wmproto.wm.protobuf.GhostSelectionMethod.GHOST_BINGO_CHALLENGE:
            {
